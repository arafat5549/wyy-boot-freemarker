...
//				$container.animate({scrollTop: scorll},500)
    ...
    var handleTreeSelect = function ($thiz, $thizVal) {
        ...
        var html = '<div id="' + name + 'TreeModal" class="modal modal-dialog fade" tabindex="-1" data-focus-on="input:first">' +
            ...
            '</div>' +
            '<div class="modal-body jqmkj-treeSelect-div albed-treeSelect-' + name + '">' +
        ...
        $modal.modal({width: dialogWidth}).off('click', '.confirm').on('click', '.confirm', function () {
            ...
            $thizVal.val(ids);
            $thiz.val(names);
            if (jqmkj.isExitsVariable(selectedValueFn) && jqmkj.isExitsFunction(selectedValueFn)) {
    ...
    var handleGridSelect = function ($thiz, $thizVal) {
        ...
        var selectedIds = jqmkj.validateNull(value) ? [] : value.split(",");
        var showNames = $thiz.val();
        var selectedNames = jqmkj.validateNull(showNames) ? [] : showNames.split(",");
        var showNodes = $thiz.attr("data-nodes");
        var selectedNodes = jqmkj.validateNull(showNodes) ? [] : JSON.parse(showNodes);
        ...
        var html = '<div id="' + name + 'GridModal" class="modal modal-dialog fade" tabindex="-1" data-focus-on="input:first">' +
            ...
            '<h4 class="modal-title">' + (title ? title : "数据选择") + '</h4>' +
            '</div>' +
            '<div class="modal-body jqmkj-gridSelect-div albed-gridSelect-' + name + '">' +
        ...
        $modal.modal({width: width}).find(".jqmkj-gridSelect-div").prepend($tempStoreForm);
            ...
            options = $.extend(true, {
                ...
                dataType: 'json',
                    ...
                    if (data && data.result && data.result.status == 1) {
                        ...
                        if(jqmkj.validateNotNull(files)){
                            for(var i=0;i<files.length; i++ ){
                                fileIds+=files[i].id+",";
                            }
                        }
                        if(jqmkj.validateNotNull(fileIds)){
...
jQuery(document).ready(function () {
    jqmkjForm.init(); // init core componets
});
