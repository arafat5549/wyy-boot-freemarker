...
var jqmkj = {
    ...
    jsonFrom: function (json, formId) {
        jqmkj.jsonFromObject(json, $("#" + formId));
    },
    jsonFromObject: function (json, $form) {
        if (json) {
            $form.find("[name]") && $form.find("[name]").each(function () {
                var name = jqmkj.toCamelCase($(this).attr('name'));
                jqmkj.setFormBoxValue($(this), json[name]);
            });
        }
    }
    /**
    ...
    jsonInitSearchFrom: function (json, formId) {
        ...
        if (json) {
            for (var i in json) {
                ...
                jqmkj.setFormBoxValue($item, json[i].value);
                if (json[i].operation == "between") {
                    jqmkj.setFormBoxValue($item.next(), json[i].endValue);
                }
            }
        }
    },
    ...
    parseJsonItemForm: function (formId) {
        return jqmkj.parseJsonItemFormTarget($(formId))
    },
    /**
     * json初始化查询表单
     *
