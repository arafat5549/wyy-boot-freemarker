        ...
        var position = el.css('position'),
            scrollParent = el.parents().filter(function () {
                ...
                return (/(auto|scroll)/).test(
                    parent.css('overflow') + parent.css('overflow-y') + parent.css('overflow-plugins.ligerui')
                );
            }).eq(0);
...
// can be called with another class, or a plain object hash containing new members.
    ...
    /* A rectangular panel that is absolutely positioned over other content
     ...
     - top
     - left
     - right (the plugins.ligerui coord of where the right edge should be. not a "CSS" right)
     - autoHide (boolean)
     - show (callback)
     - hide (callback)
     */
    ...
    /* A "coordinate map" converts pixel coordinates into an associated cell, which has an associated date
     ...
     CoordMap.prototype = {
     build: function() {},
     getCell: function(plugins.ligerui, y) {}
     };
    ...
    DayGrid.mixin({
        ...
        renderSegPopoverContent: function (cell, segs) {
            ...
            var content = $(
                ...
                '<span class="fc-close ' +
                (isTheme ? 'ui-icon ui-icon-closethick' : 'fc-icon fc-icon-plugins.ligerui') +
                '"></span>' +
                '<span class="fc-title">' +
                htmlEscape(title) +
                '</span>' +
