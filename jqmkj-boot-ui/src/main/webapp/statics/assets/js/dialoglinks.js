    ...
    return {
        ...
        initPage: function () {
            ...
            $(document).off("click.pageModal").on("click.pageModal", "a.pageModal", function () {
                ...
                var loadUrl = el.attr('data-url'), loadUrlName = el.attr('data-url-name'),
                ...
                jqmkj.setUserCookie("module-load-url-name", loadUrlName);
                try {
                    $modal.load(loadUrl, function () {
                        $('body').modalmanager('removeLoading');
                        //console.log("$modal.text()=",$modal.text())
            ...
            var requestUrl = location.href,
                loadUrlName = requestUrl.substring(requestUrl.lastIndexOf("#") + 1, requestUrl.length);
            if (!loadUrlName) {
                loadUrlName = jqmkj.getUserCookie("module-load-url-name");
            }
            if (loadUrlName) {
                var $targetItem = $("a.pageModal[data-url-name='" + loadUrlName + "']");
                if ($targetItem && $targetItem.length > 0) {
                    $targetItem.trigger("click");
                } else if (!loadUrlName.endWith('/index')) {
                    loadUrlName = jqmkj.getUserCookie("module-load-url-name");
            ...
            $(document).off("click.dialog").on("click.dialog", "a.dialog", function () {
                ...
                $modal.load(loadUrl + (loadUrl.indexOf("?") == -1 ? "?" : "&") + 'isModal=' + isModal, function () {
                    ...
                    $targetEvent.off('click', '.add, .update').on('click', '.add, .update', function () {
                        var operation = $(this).hasClass("add") ? 'add' : 'update',
                            $form = $targetEvent.find('.form-validation'), validateFun = $form.attr("validateFun"),
                            flag = true;
                        if (FormValidation.validate()) {
                            jqmkj.isExitsFunction(validateFun) && eval("flag = " + validateFun + "()");
                            if (flag) {
            ...
            if (re && el) {
                ...
                if (re.status == "1" && (tableId || refresh)) {
                    if (refresh) {
                        ...
                        if (jqmkj.isExitsFunction(ajaxReloadAfterFu)) {
...
//                        url: "https://api.github.com/search/repositories",
            ...
            // (was #select2-drop in Select2 v3.plugins.ligerui, in Select2 v4 can be selected via
