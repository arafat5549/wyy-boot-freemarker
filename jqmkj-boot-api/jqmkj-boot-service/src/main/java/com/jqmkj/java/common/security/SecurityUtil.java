...
import com.jqmkj.java.modules.sys.domain.*;
import com.jqmkj.java.modules.sys.repository.UserRepository;
import com.jqmkj.java.modules.sys.service.*;
...
public final class SecurityUtil {
    ...
    public static JqmkjProperties jqmkjProperties = SpringContextHolder.getBean(JqmkjProperties.class);
    ...
    public static User getByLoginId(final String loginId) {
        ...
        if (user == null||PublicUtil.isEmpty(user.getId())) {
            user = userService.findOneByLoginId(loginId).map(u -> {
                ...
                Assert.assertIsTrue(jqmkjProperties.getUserType().contains(u.getType()),"用户 " + loginId + " 无法登录" );
    ...
    public static List<QueryCondition> dataScopeFilterSql(String orgAlias, String userAlias) {
        if(jqmkjProperties.getTestMode()){
            return null;
        }
