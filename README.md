# JqmkjBoot 企业信息化快速开发平台

## 平台简介

JqmkjBoot是基于多个优秀的开源项目(jhipster,jeesite)，高度整合封装而成的高效，高性能，强安全性的**开源**Java EE快速开发平台。

JqmkjBoot是您快速完成项目的最佳基础平台解决方案，JqmkjBoot是您想学习Java平台的最佳学习案例，JqmkjBoot还是接私活的最佳助手。

JqmkjBoot是在Spring Boot基础上搭建的一个Java基础开发平台，以Spring MVC为模型视图控制器，MyBatis为数据访问层，
SpringSecurity为权限授权层，Ehcahe对常用数据进行缓存，是JavaEE界的最佳整合。

JqmkjBoot主要定位于企业信息化领域，已内置企业信息化系统的基础功能和高效的**代码生成**工具，
包括：系统权限组件、数据权限组件、数据字典组件、核心工具组件、视图操作组件、工作流组件、代码生成等。
前端界面风格采用了结构简单、性能优良、页面美观大气的METRONIC Bootstrap页面展示框架。
采用分层设计、双重验证、提交数据安全编码、密码加密、访问验证、数据权限验证。
使用Maven做项目管理，提高项目的易开发性、扩展性。

JqmkjBoot目前包括以下两大模块，系统管理（SYS）模块、**系统管理模块** ，包括企业组织架构（用户管理、机构管理、区域管理）、
菜单管理、角色权限管理、字典管理，系统监控，回话管理，接口管理等功能； **代码生成模块** ，完成重复的工作。

JqmkjBoot 提供了常用工具进行封装，包括日志工具、缓存工具、服务器端验证、数据字典、当前组织机构数据
（用户、机构、区域）以及其它常用小工具等。另外还提供一个强大的在线 **代码生成** 工具，
此工具提供简单的单表、一对多、树结构功能的生成，如果对外观要求不是很高，生成的功能就可以用了。
如果你使用了JqmkjBoot基础框架，就可以很高效的快速开发出，优秀的信息管理系统。
...
cloud版本发布，基于[jhipster-registry](https://github.com/somewhereMrli/jhipster-registry)为注册中心。 同时也可以docker环境搭建，基于docker-compose启动，
配置文件位于jqmkj-boot-cloud/docker/jhipster-registy.yml
1.	启动cloud前请升级数据库，重新执行jqmkj-new.sql
2.	使用docker-compose命令启动jqmkj-boot-cloud/docker/jhipster-registry.yml
3.	启动jqmkj-boot-cloud/jqmkj-boot-cloud-micro JqmkjBootCloudMicro
4.	启动jqmkj-boot-cloud/jqmkj-boot-cloud-gateway JqmkjBootCloudGateway
...
2. 导入ide前，安装lombok插件
3. 修改jqmkj-boot-web-starter src\main\resources\config\application-dev.yml文件中的数据库设置参数。
4. 根据修改参数创建对应MySql或Oracle数据库用户和参数。
5. 运行jqmkj-new.sql脚本初始化数据库
6. 最高管理员账号，用户名：admin 密码：admin 

## 常见问题

1. 用一段时间提示内存溢出，请修改JVM参数：-Xmx512m -XX:MaxPermSize=256m
2. 如果坚持使用hibernate版本，请切换分支 jqmkj-boot-hibernate

## 如何交流、反馈、参与贡献？

* E-mail：837158334@qq.com
* GitHub：<https://github.com/somewhereMrli/jqmkj-boot>
* JqmkjBoot-QQ交流群：685728393
...
6. 你可以二次包装出售，**但还请保留文件中的版权和作者信息**，并在你的产品说明中注明JqmkjBoot。
