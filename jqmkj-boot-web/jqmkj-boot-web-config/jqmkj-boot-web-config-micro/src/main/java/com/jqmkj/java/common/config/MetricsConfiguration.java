...
import com.jqmkj.java.common.config.metrics.SpectatorLogMetricWriter;
...
public class MetricsConfiguration extends MetricsConfigurerAdapter {
    ...
    public MetricsConfiguration(JqmkjProperties jqmkjProperties) {
    ...
    public void init() {
        ...
        if (jqmkjProperties.getMetrics().getJmx().isEnabled()) {
            log.debug("Initializing Metrics JMX reporting");
            JmxReporter jmxReporter = JmxReporter.forRegistry(metricRegistry).build();
            jmxReporter.start();
        }
        if (jqmkjProperties.getMetrics().getLogs().isEnabled()) {
            ...
            final Slf4jReporter reporter = Slf4jReporter.forRegistry(metricRegistry)
                    ...
                    .build();
            reporter.start(jqmkjProperties.getMetrics().getLogs().getReportFrequency(), TimeUnit.SECONDS);
        }
    }

    /* Spectator metrics log reporting */
    @Bean
    @ConditionalOnProperty("jqmkj.logging.spectator-metrics.enabled")
    @ExportMetricReader
